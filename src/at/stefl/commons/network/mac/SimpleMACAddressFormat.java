...
public class SimpleMACAddressFormat extends MACAddressFormat {
    ...
    public StringBuffer format(MACAddress address, StringBuffer toAppendTo,
        ...
        for (int i = 0; i < pattern.length(); i++) {
            ...
            if (Character.toLowerCase(c) == 'x') {
                ...
                else hexChar = hexValue.charAt(hexIndex + hexValue.length()
                        - DIGIT_COUNT);
                
                if (Character.isUpperCase(c)) hexChar = Character
                        .toUpperCase(hexChar);
    ...
    public MACAddress parseObject(String source, ParsePosition pos) {
        ...
        if (source.length() != pattern.length()) throw new IllegalArgumentException(
        ...
        for (int i = 0; i < source.length(); i++) {
            ...
            if (Character.toLowerCase(patternChar) == 'x') hexString += c;
