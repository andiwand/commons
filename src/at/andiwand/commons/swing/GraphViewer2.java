...
import at.andiwand.common.util.AWTUtil;


// TODO: make use of Vertex2d
public class GraphViewer2 extends JComponent {
	...
	public class MouseHandler extends MouseAdapter {
		...
		private Set<GraphViewerVertex> oldSelection;
		...
		public void mousePressed(MouseEvent e) {
			...
			oldSelection = new HashSet<GraphViewerVertex>(selectedVertices);
			
			if (movingVertex != null) {
				handleSelection(movingVertex, e.isControlDown(), oldSelection);
				return;
			}
			
			if (selectionMode != SELECTION_MODE_MULTI) return;
			selectionStart = e.getPoint();
			selectionRectangle = new Rectangle(e.getPoint());
		}
		
		public void mouseDragged(MouseEvent e) {
			if (movingVertex != null) {
				Point point = new Point(e.getPoint().x + movingDx,
						e.getPoint().y + movingDy);
				layout.moveVertex(movingVertex, point);
			} else if ((selectionMode == SELECTION_MODE_MULTI)
					&& (selectionRectangle != null)) {
				selectionRectangle = AWTUtil.create(selectionStart,
						e.getPoint());
				
				Set<GraphViewerVertex> selection = new HashSet<GraphViewerVertex>();
				
				synchronized (vertexMap) {
					for (GraphViewerVertex vertex : vertexMap.values()) {
						if (selectionRectangle.contains(vertex.getBounds()))
							selection.add(vertex);
					}
				}
				
				handleSelection(selection, e.isControlDown(), oldSelection);
			} else {}
	...
	public GraphViewer2(Graph<? extends Object, ? extends Edge> graph) {
		this();
	...
	private GraphViewerVertexFactory getSuitableVertexFactory(
		...
		synchronized (vertexFactories) {
			for (GraphViewerVertexFactory factory : vertexFactories) {
				Class<? extends Object> factoryVertexClass = factory.getVertexClass();
	...
	private void addVertex(Object vertex) {
		GraphViewerVertexFactory factory = getSuitableVertexFactory(vertex.getClass());
	...
	private void clearSelection() {
		synchronized (selectedVertices) {
			for (GraphViewerVertex selectedVertex : selectedVertices)
	...
	private void handleSelection(GraphViewerVertex vertex, boolean xorMode,
			Set<GraphViewerVertex> oldVertices) {
		if (vertex == null) return;
		
		Set<GraphViewerVertex> vertices = new HashSet<GraphViewerVertex>();
		vertices.add(vertex);
		handleSelection(vertices, xorMode, oldVertices);
	}
	
	private void handleSelection(Set<GraphViewerVertex> vertices,
			boolean xorMode, Set<GraphViewerVertex> oldVertices) {
		synchronized (selectedVertices) {
			if (xorMode) {
				Set<GraphViewerVertex> addedVertices = new HashSet<GraphViewerVertex>(
						vertices);
				addedVertices.removeAll(oldVertices);
				
				Set<GraphViewerVertex> removedVertices = new HashSet<GraphViewerVertex>(
						vertices);
				removedVertices.retainAll(oldVertices);
				
				selectedVertices.addAll(addedVertices);
				selectedVertices.removeAll(removedVertices);
				
				for (GraphViewerVertex vertex : addedVertices)
					vertex.setSelected(true);
				for (GraphViewerVertex vertex : removedVertices)
					vertex.setSelected(false);
			} else {
				selectedVertices.addAll(vertices);
				for (GraphViewerVertex selectedVertex : vertices)
