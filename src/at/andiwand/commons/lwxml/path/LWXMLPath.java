package at.andiwand.commons.lwxml.path;
...
public class LWXMLPath {
	...
	for (int i = 1; i <= path.getDepth(); i++) {
	    if (!getNodeIdentifier(getDepth() - i).equals(
		    path.getNodeIdentifier(path.getDepth() - i)))
		return false;
	}
