...
public class CharArrayQueue extends AbstractQueue<Character> implements
	...
	if (tmp < size)
	    System.arraycopy(buffer, 0, cbuf, tmp, size - tmp);
	...
	if (len > (buffer.length - index(size)))
	    System.arraycopy(cbuf, off + tmp, buffer, 0, len - tmp);
	size += len;
	...
	if (charSequence.length() > (buffer.length - size))
	...
	if (tmp < size)
	    System.arraycopy(buffer, 0, result.buffer, tmp, size - tmp);
	result.size = size;
